...
<div class="min-h-screen flex items-center justify-center bg-gray-100 py-8 px-4">
    <div class="w-full max-w-md bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-2xl font-bold text-center text-gray-800 mb-6">Tambah Pertanyaan</h2>
        ...
        <form action="{{ route('pertanyaan.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="kategori" id="kategori" required
                    ...
                    <option value="E" {{ old('kategori') == 'E' ? 'selected' : '' }}>E (Emosional)</option>
                    <option value="P" {{ old('kategori') == 'P' ? 'selected' : '' }}>P (Teman Sebaya)</option>
                    <option value="H" {{ old('kategori') == 'H' ? 'selected' : '' }}>H (Hiperaktivitas)</option>
                    <option value="C" {{ old('kategori') == 'C' ? 'selected' : '' }}>C (Perilaku)</option>
                    <option value="Pro" {{ old('kategori') == 'Pro' ? 'selected' : '' }}>Pro (Prososial)</option>
                </select>
                @error('kategori')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
